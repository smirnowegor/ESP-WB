...
# 3) Удаление предыдущих установок
...
$SUDO rm -f "${DOWNLOAD_DIR}/${FOUND_FNAME}" || true
if ! wget --progress=bar:force -O "${DOWNLOAD_DIR}/${FOUND_FNAME}" "$FOUND_URL"; then
...
if command -v apt-get >/dev/null 2>&1; then
  if ! $SUDO apt-get install -y "${DOWNLOAD_DIR}/${FOUND_FNAME}"; then
